--thecall.lua by Cannonballdex
-- Essedera in The Eastern Wastes

local mq = require('mq')

local START_ZONE_ID = 824
local TASK_ZONE_ID = 831
local TASK_NAME = "The Call"
local TASK_TIMER_THRESHOLD = 19500000
...
local function dropcampfire()
    local golem = mq.TLO.Spawn("a worried golem")

    if golem() ~= nil
        and golem.Distance() > 150
        and inZone(TASK_ZONE_ID)
...
local function moveup()
    local golem = mq.TLO.Spawn("a worried golem")
    if inZone(TASK_ZONE_ID) and golem() ~= nil and golem.Distance() < 50 then
        if not waitForGroupWithin(GROUP_STEP_RADIUS, GROUP_WAIT_TIMEOUT_MS, 'saying retreat') then
            return
        end

        mq.delay(5000)
        mq.cmd('/dgga /target a worried golem')
        mq.delay(5000)
        mq.cmd('/dgga /makemevisible')
        mq.delay(500)
        mq.cmd('/dgga /say retreat')
    end
end
...
local function checktask()
    ...
    mq.cmd('/dgga /nav spawn Essedera')
    mq.delay(1000)
    mq.cmd('/dgga /target Essedera')
    mq.delay(1000)
